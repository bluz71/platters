class App.AlbumsSelect
  ...
  # Disable empty, or default, Album filter form values, doing so will declutter
  # the URL (and params hash) of unnecessary GET parameters.
  filterSubmit: (event) ->
    $("[name=genre]").prop("disabled", true) unless $("[name=genre]").val()
    $("[name=year]").prop("disabled", true)  unless $("[name=year]").val()
